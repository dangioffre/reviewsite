            ...
            <form action="{{ route('reviews.update', $review) }}" method="POST" class="space-y-8">
                ...
                <div class="bg-gradient-to-br from-[#27272A] to-[#1A1A1B] rounded-2xl shadow-2xl border border-[#3F3F46] p-8">
                    ...
                    <div class="grid md:grid-cols-2 gap-6">
                        ...
                        @if($review->product->type === 'game')
                            ...
                            <div>
                                ...
                                <select id="game_status" 
                                    ...
                                    <option value="want" {{ old('game_status', $review->game_status) === 'want' ? 'selected' : '' }}>Want to Play</option>
                                    <option value="playing" {{ old('game_status', $review->game_status) === 'playing' ? 'selected' : '' }}>Currently Playing</option>
                                    <option value="played" {{ old('game_status', $review->game_status) === 'played' ? 'selected' : '' }}>Completed</option>
                                </select>
                                @error('game_status')
                                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                        ...
                        <!-- Platform Played On -->
                        <div class="{{ $review->product->type === 'game' ? '' : 'md:col-span-2' }}">
