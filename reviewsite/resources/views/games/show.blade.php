                ...
                <div class="lg:col-span-1">
                    <div class="sticky top-8">
                        ...
                        <div class="bg-gradient-to-br from-[#27272A] to-[#1A1A1B] rounded-2xl p-8 border border-[#3F3F46] shadow-2xl">
                            ...
                            <form action="{{ route('games.reviews.store', $product) }}" method="POST" class="space-y-6">
                                @csrf
                                
                                <div>
                                    <label class="block text-white font-semibold mb-2 font-['Inter']">Rating</label>
                                    <select name="rating" required class="w-full bg-[#1A1A1B] border border-[#3F3F46] rounded-lg px-4 py-3 text-white font-['Inter'] focus:border-[#2563EB] focus:ring-[#2563EB]">
                                        <option value="">Select Rating</option>
                                        @for($i = 10; $i >= 1; $i--)
                                            <option value="{{ $i }}">{{ $i }}/10 - {{ $i >= 9 ? 'Excellent' : ($i >= 7 ? 'Great' : ($i >= 5 ? 'Good' : ($i >= 3 ? 'Fair' : 'Poor'))) }}</option>
                                        @endfor
                                    </select>
                                </div>
                                
                                <div>
                                    <label class="block text-white font-semibold mb-2 font-['Inter']">Your Review</label>
                                    <textarea 
                                        name="review" 
                                        required 
                                        rows="6" 
                                        placeholder="Share your thoughts about this game..."
                                        class="w-full bg-[#1A1A1B] border border-[#3F3F46] rounded-lg px-4 py-3 text-white placeholder-[#A1A1AA] font-['Inter'] focus:border-[#2563EB] focus:ring-[#2563EB] resize-none"
                                    ></textarea>
                                </div>
