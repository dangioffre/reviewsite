<div class="bg-gradient-to-br from-[#27272A] to-[#1A1A1B] rounded-2xl shadow-2xl border border-[#3F3F46] w-full max-w-lg p-6 relative">
    <!-- Close Button -->
    <button @click="$dispatch('close-modal')" class="absolute top-4 right-4 text-[#A1A1AA] hover:text-white transition-colors">
    ...
    <!-- Debug Test Button -->
    <div class="mb-4 p-3 bg-[#7C3AED]/20 border border-[#7C3AED]/30 rounded-lg">
        <button wire:click="testButton" class="w-full bg-[#7C3AED] text-white py-2 px-4 rounded font-semibold">
            TEST BUTTON - Click Count: {{ $testCount ?? 0 }}
        </button>
    </div>
